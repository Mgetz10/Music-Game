...
class Note {
  ...
  isSameLineDifNote() {
    return this.staffLine.firstChild.className.includes('sharp');
  }
  ...
  gamePlay() {
    if (this.generated) {
      ...
      if (noteGenerated) {
        leaves[scoreCounter].classList.remove(
          `sprout-${leafNumber[scoreCounter]}`
        );
      ...
      if (scoreCounter > 0) {
        ...
        scoreCounter--;
        leaves[scoreCounter].classList.remove(
          `leaf-${leafNumber[scoreCounter]}`
        );
        leaves[scoreCounter].classList.add(
          `sprout-${leafNumber[scoreCounter]}`
        );
      }
...
const playSong = (song, delay) => {
  ...
  const songInterval = setInterval(() => {
    if (placeInSong > 0) {
      song[placeInSong - 1].name.noteStop();
    }
...
const switchKeyHelpToggle = () => {
  ...
  // for (const compKeyLetter of compKeyLetters) {
  //   compKeyLetter.classList.toggle('hide');
  // }
};
...
const introSlide = () => {
  ...
  if (slideNumber === 12) guessGameExTwo.classList.add('hide');
...
nextBtnEleven.addEventListener(
  'click',
  () => {
